...
1.  **Enter Download Mode:** With the device powered off, hold **Volume Down + Bixby** and connect the USB cable to the PC .
2.  **Navigate the Warning:** The screen will show a warning. Since the touch screen is unresponsive, use the **Volume keys** to navigate and the **Bixby button** (or Power button, depending on prompt) to confirm. The exact mapping may require experimentation. The goal is to select the option to unlock the bootloader, which will wipe the device.
3.  **Initial Reboot:** The device will reboot. It might prompt for a factory reset, which you can confirm using the hardware keys again.
4.  **Re-verify OEM Unlocking:** After a minimal initial setup (navigating blindly), try to re-enable Developer Options and ensure "OEM Unlock" is still enabled. This step is difficult without a display; success means the bootloader is confirmed unlocked.

#### 1.3 Flashing LineageOS Recovery

1.  **Boot into Download Mode Again:** Repeat the key combination (Volume Down + Bixby + USB cable) .
2.  **Flash Recovery using Heimdall:** On your PC, execute the following command in the directory containing `recovery.img`:
    ```bash
    heimdall flash --RECOVERY recovery.img --no-reboot
    ```
    The `--no-reboot` flag is crucial .
3.  **Immediate Reboot to Recovery (Critical Step!):** This is the most timing-sensitive part. After the flash completes, the phone will still show "Downloading...". You must **immediately** force a reboot into recovery before the system can overwrite the freshly flashed recovery.
...
1.  **Navigate Recovery with Volume Keys:** In the recovery, use the **Volume keys** to move up/down and the **Power button** to select.
2.  **Wipe Data:** Navigate to "Factory Reset" → "Format data / factory reset" and confirm. This is necessary to remove encryption.
3.  **Enable ADB Sideload:** Go back to the main menu, select "Apply Update" → "Apply from ADB".
4.  **Sideload from PC:** On your PC, run:
    ```bash
    adb -d sideload /path/to/lineage-*.zip
    ```
    The process will show progress on the PC. It may stall at 47%, which is normal .
5.  **Reboot:** Once complete, navigate back and select "Reboot system now". The first boot will take several minutes.
...
1.  **Find the Phone's IP:** On your router's DHCP client list or using a network scanner, find the IP address assigned to your S10+.
2.  **Connect via SSH:**
    ```bash
...
1.  Obtain an API key from [NVIDIA NIM](https://build.nvidia.com/).
2.  Verify available models to ensure you have the correct name:
    ```bash
...
```json
{
  ...
  "models": {
    ...
    "providers": {
      ...
      "ollama": {
        "baseUrl": "http://192.168.0.***:11434/v1",   // Replace with your phone's actual IP
        "apiKey": "dummy",
        "api": "openai-completions",
        "models": [
          {
...
```bash
ssh -N -L *****:127.0.0.1:***** -p 80** username@phone-ip
```

Then open `http://localhost:*****` in your PC's browser and enter the gateway token.
...
| Purpose                                 | Command                                                                                 |
|-----------------------------------------|-----------------------------------------------------------------------------------------|
| SSH into phone                          | `ssh -p 80** username@phone-ip`                                                         |
| SSH tunnel for dashboard                | `ssh -N -L 18789:127.0.0.1:***** -p 80** username@phone-ip`                             |
| X11 forwarding (VcXsrv on Windows)      | `ssh -X -p 80** username@phone-ip` then run `firefox &`                                 |
...
### Hardware for Running Larger Local LLMs
...
- All IP addresses, API keys, and tokens in this document are placeholders. Replace them with your own values.
- The configuration assumes **Ollama is running on the phone itself**. To use a more powerful PC for the local model, set up a **reverse SSH tunnel** (`-R`) as described in the provided notes.
- Always **validate your JSON** after editing `openclaw.json` using an online validator like [jsonlint.com](https://jsonlint.com/).
- After every LineageOS update, you may need to reapply some system-level tweaks (like custom DNS settings) .

---

**Enjoy your self-hosted, private AI assistant!**  
